...
homekit_accessory_t *accessories[] = {
        HOMEKIT_ACCESSORY(.id=1, .category=homekit_accessory_category_thermostat, .services=(homekit_service_t*[]){
                HOMEKIT_SERVICE(ACCESSORY_INFORMATION, .characteristics=(homekit_characteristic_t*[]){
                        HOMEKIT_CHARACTERISTIC(NAME, "Cozinha AC"),
                        &manufacturer,
                        &serial,
                        &model,
                        &revision,
                ...
                HOMEKIT_SERVICE(FAN2, .characteristics=(homekit_characteristic_t*[]) {
                        HOMEKIT_CHARACTERISTIC(NAME, "Velocidade"),
                        &fan_active,
                        &fan_rotation_speed,
                        &fan_swing_mode,
                        NULL
...
void create_accessory_name() {
        ...
        int name_len = snprintf(NULL, 0, "kitchen AC-%02X%02X%02X",
                                macaddr[3], macaddr[4], macaddr[5]);
        char *name_value = malloc(name_len+1);
        snprintf(name_value, name_len+1, "kitchen AC-%02X%02X%02X",
                 macaddr[3], macaddr[4], macaddr[5]);
...
void user_init(void) {
        ...
        wifi_config_init("kitchen-ac", NULL, on_wifi_ready);
